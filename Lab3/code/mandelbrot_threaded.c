...
pthread_mutex_t lock; //Definimos el pthread_mutex
...
int main(void) {
  ...
  unsigned int arguments[] = {(unsigned int) pixels,width,height}; //<---- ?

  startTimer(0);
  for(int i = 0; i < R*R; i++){
    printf("Creating thread [%d]\n",i);
    pthread_create(&tids[i],NULL,(void *) &generate_mandelbrot,&arguments);
  }
